package com.micro.old.server.service.impl;
...
public class BullfightGameServiceImpl implements BullfightGameService {
	...
	public void connectWebsocket(String msgType, String token, String userMsg, ChannelHandlerContext ctx) {
		...
		GameRequestVO webSocketRequest = (GameRequestVO) JsonUtil.parseObject(JsonUtil.parseJsonString(map),
				GameRequestVO.class);
	...
	public GlobeResponse enterGame(GameRequestVO enterBullfight, ChannelHandlerContext ctx) {
		return null;
		/*
		 * String userMsg = enterBullfight.getUserMsg(); UserInputVo inputVo =
		 * JsonUtil.parseObject(userMsg, UserInputVo.class); Long siteId =
		 * inputVo.getSiteId(); String userName = inputVo.getAccount();
		 * 
		 * GlobeResponse<Object> globeResponse;
		 * 
		 * if(inputVo.getIdentity().equals(1)) { globeResponse =
		 * accountFeignClient.getUserMoney(userName, siteId);
		 * 
		 * if (!globeResponse.getCode().equals("200")) { GlobeResponse gr = new
		 * GlobeResponse(); gr.setGlobeResponse(globeResponse.getCode(),
		 * globeResponse.getMsg()); return gr; }
		 * 
		 * TbAccAccount account =
		 * JsonUtil.parseObject(JsonUtil.parseJsonString(globeResponse.getData()),
		 * TbAccAccount.class); inputVo.setMoney(account.getMoney());
		 ...
		 * //将当前客户端与游戏房间绑定 if(globeResponse.getCode().equals("200")) { String roomNum =
		 * ((Map)globeResponse.getData()).get("roomNum").toString(); Object object =
		 * ((Map)globeResponse.getData()).get("clearToken"); if(object != null) {
		 * //将之前的socket连接删除 String clearBefore =
		 * ((Map)globeResponse.getData()).get("clearToken").toString(); Map<String,
		 * Object> stringMap = new HashMap<>(); stringMap.put("token", clearBefore);
		 * 
		 * }
		 * 
		 * 
